...
<template>
    <section class="flex-1 flex flex-col gap-5 bg-custom-purple justify-between">
        ...
        <form @submit.prevent="handleRegister" class="p-5 flex gap-2 flex-col bg-white overflow-hidden rounded-t-3xl">
            ...
            <small class="block text-light-gray my-5 text-xs text-center">Tu as déjà un compte ? <NuxtLink :to="{ name: 'login' }" class="font-semibold text-custom-purple">Me connecter</NuxtLink></small>
        </form>
    </section>
</template>
