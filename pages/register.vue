  ...
  <section class="flex-1 flex flex-col gap-5 bg-custom-purple justify-between">
    ...
    <form @submit.prevent="handleRegister" class="p-5 flex gap-2 flex-col bg-white overflow-hidden rounded-t-3xl">
      <div class="mb-5">
        ...
        <h2 class="font-medium text-light-gray text-center mt-2 text-sm">Une question ? Toutes les réponses se trouvent
          ici 👇</h2>
      </div>
      <div class="flex items-center justify-center gap-2">
        <label for="firstname">
          <input v-model="form.firstname" id="firstname"
                 ...
                 type="text" inputmode="text" placeholder="Prénom">
        </label>
        <label for="lastname">
          <input v-model="form.lastname" id="lastname"
                 class="font-medium text-sm w-full py-4 px-5 caret-custom-purple rounded-2xl border border-solid border-custom-light-gray focus:border-custom-purple bg-custom-light-gray outline-none"
                 type="text" inputmode="text" placeholder="Nom">
        </label>
      </div>
      <label for="email">
      ...
      <small class="block text-light-gray my-5 text-xs text-center">Tu as déjà un compte ?
        <NuxtLink :to="{ name: 'login' }" class="font-semibold text-custom-purple">Me connecter</NuxtLink>
      </small>
    </form>
  </section>
</template>
