    ...
    <section class="flex-1 p-5 flex items-center justify-center">
        <form @submit.prevent="onSubmit" class="p-5 flex gap-2 flex-col bg-white shadow-sm rounded-3xl overflow-hidden">
            ...
            <label for="description">
                <textarea v-model="description" id="description" rows="5" class="resize-none font-medium w-full py-4 px-5 caret-custom-purple rounded-2xl border border-solid border-custom-light-gray focus:border-custom-purple bg-custom-light-gray outline-none" type="text" inputmode="text" placeholder="Description"></textarea>
            </label>
            <small v-if="false" class="block text-custom-purple text-sm text-center">Mot de passe oublié ?</small>
            <button type="submit" :disabled="description === ''" :class="description !== '' ? 'bg-custom-purple text-white' : 'bg-transparent text-light-gray'" class="w-full flex items-center justify-center gap-2 font-semibold mt-5 py-4 px-5 rounded-2xl border border-solid border-light-gray">
